...
namespace KKCarpoolMNGMNT.Controlers
{
    public class HomeController : Controller
    {
        ...
        public IActionResult SaveRide([FromBody] TravelPlanDTO data)
        {
            ...
            for(int i=0; i < data.ListOfPassengersIds.Length; i++)
            {
				for (int j = i + 1; j < data.ListOfPassengersIds.Length; j++)
				{
					if (data.ListOfPassengersIds[i] == data.ListOfPassengersIds[j])
					{
						data.Error = "You added an employee who is already in the car";
					}
				}
			}

            if (data.ListOfPassengersIds.Length > 0)
            {
            ...
            {
				data.Error = "You are not added employees ";
				return Json(data);
			}
